...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:variable name="nsd" select="translate( count( /report/incident ) cast as xs:string, '123456789','000000000')"/>
  ...
  <xd:doc>
    <xd:desc>When we find the root report element, <xd:ol>
      ...
      position. (This is the "addCnt" mode.)</xd:li>
  ...
  <xsl:template match="/report">
    <xsl:variable name="report">
      <xsl:apply-templates select="." mode="addCnt"/>
    </xsl:variable>
  ...
  <xd:doc>
    <xd:desc>In first pass (mode addCnt) most everything just gets copied.</xd:desc>
  </xd:doc>
  <xsl:template match="@*|node()" mode="addCnt">
    <xsl:copy>
      <xsl:apply-templates select="@*|node()" mode="#current"/>
    </xsl:copy>
  </xsl:template>
  <xd:doc>
    <xd:desc>The exception in the first pass (mode addCnt) is that incident elements
    get decorated with a new @pos attribute that gives their ordinal number within
    the input file.</xd:desc>
  </xd:doc>
  <xsl:template match="incident" mode="addCnt">
    <xsl:copy>
  ...
  <xd:doc>
    ...
    fields of itnerest separated by tabs.</xd:desc>
  </xd:doc>
  <xsl:template match="incident">
    <xsl:text>&#x0A;</xsl:text>
    <xsl:value-of select="format-number( @pos, $nsd )"/>
    <xsl:text>&#x09;</xsl:text>
    ...
    <xsl:text>&#x09;</xsl:text>
    <xsl:value-of select="location/start/line"/>
    <xsl:text>&#x09;</xsl:text>
    <xsl:value-of select="location/start/col"/>
    <xsl:text>&#x09;</xsl:text>
    <xsl:value-of select="location/end/line"/>
    <xsl:text>&#x09;</xsl:text>
