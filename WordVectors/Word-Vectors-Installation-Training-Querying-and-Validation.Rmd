...
If you are running code in a block line by line, your cursor will automatically go down to the next line after each is run, so you can move through the block by repeatedly hitting `command-return` or `control-return`.
...
Once the packages have been installed, you should load them using the `library()` function. You only need to install packages once, but you'll have to load them every time you start a new session in RStudio. You'll want to make a habit of checking your working directory and loading your packages at the start of each session.
