...
These walkthroughs provide code and instructions for training and querying models using the Gensim instantiation of Word2Vec. The walkthroughs also cover fundamental concepts in Python. These walkthroughs were developed in 2023 and updated in 2025. 

The code has been tested as of July 2025, but due to rapid change in the Python libraries at stake, there is a possibility you may run into errors or warnings that we were not able to anticipate. If you encounter any problems, and you're comfortable doing so, please file a [GitHub issue](https://github.com/NEU-DSG/wwp-public-code-share/issues). 

## Walkthrough Files

This directory contains five Jupyter Notebook files, each of which contains a combination of prose and executable code cells. 

If you are new to Python, we recommend going through the Notebooks in the order below. If you are familiar with Python, you can skip to the [Introduction to Word Vectors in Python](word2vec-fundamentals.ipynb).
...
In addition to the four Jupyter Notebook files, the directory also contains four sub-directories. 
...
The directory also includes a "models" sub-directory within which are sample model and .wordvectors files for models trained on the corpus of recipes ("recipes-demo.model" and "recipes-demo.wordvectors") and on the Women Writers Online collection ("wwo-demo.model" and "wwo-demo.wordvectors"). All models produced by the notebooks should be saved to this folder.

Finally, the directory includes an "output" sub-directory in which any outputs from working with the models can be saved and a "testing" folder containing the word list used to evaluate the models ("analogies_test.txt").
