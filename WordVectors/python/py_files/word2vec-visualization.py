...
# the (111) here just means generate a 1 X 1 plot
...
# # tSNE Analysis ##
# 
# The final form of mathematical analysis that we will cover in this tutorial is tSNE analysis. T-distributed Stochastic Neighbourhood Embedding (tSNE) is a dimensionality reduction algorithm similar to PCA. However, while PCA is more concerned with preserving variance in a data set, tSNE cares more about things that are close together. Essentially, tSNE is a type of analysis that allows you to visualize all of the words in your corpus by according to their relationship with other words. Imagine that tSNE is visualizing your corpus as a neighborhood where some words live next door to one another and other words might live across the street or on a different street altogether. Another important difference between tSNE and PCA, is that the results of tSNE analysis vary with each run. This is because tSNE is a probabilistic technique. tSNE is also always working in a two-dimensional space whereas PCA can work with many dimensions. 
# 
