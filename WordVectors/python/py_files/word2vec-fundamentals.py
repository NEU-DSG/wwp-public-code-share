...
# 
# In addition to allowing you to ask interesting questions of your textual data (for instance, what word is most similar to "king"), word embeddings have other uses in natural language processing. For instance, a word embedding model can be used for natural language processing tasks such as text classification. Because word embeddings capture the semantic use of a word, many natural language processing tasks become much easier with a model trained on word vectors. Word embedding models can also help us understand how language works in a set of texts. For instance, what if you wanted to know what words are most often used in the same contexts as "women" and "girls" in magazines aimed at men or women? Word embedding models would be an excellent methodological choice. 
# 
...
# - **glob** The glob library allows us to access files based on their filetype. This will be useful for loading a set of models into memory
# - **Path** The Path library gives us access to files in other directories besides our current working directory
# - **gensim** Gensim is the library which contains the instance of Word2Vec that we are using 
...
# A good practice in programming is to place your import statements together at the top of your code
...
# 
# 2. You can use **tab completion** to fill in your file paths. Within the quotation marks (don't delete these!), hit the `tab` key and it will bring up the folders and files at your specific location. You can enter `../` to go up a directory. Each time you enter `../` it will go up one folder level in your computer, and you can then use `tab` to check where you are. Once you get **up** to, say, your "Documents" folder, you can then use `tab` to go **down** into the folder with your files. Entering the start of the name of the folder you want after you hit `tab` will narrow your results to make the navigation a bit more efficient. Even if you are not used to filling in file paths, you can use this combination of `tab` and `../` to navigate to the folder with the files that you want to use.
# 
...
# ## Code for Loading Texts from a Folder
# 
...
# 
# By default, the code will run on the sample folder of recipes published with this tutorial; if you just want to see what it looks like to train a model, you don't need to change anything in the code below. 

# In[ ]:


dirpath = r'./data/sample-data-recipes/' # get file path (change this to the folder with your texts!)
...
# 
# ### The Code for Cleaning Textual Data ###
# 
...
def clean_text(text):
    ...
    # Cleans the given text using regular expressions to split and lower-case versions to create
...
# 
# If you loaded your texts in from a dataframe, instead of as a set of text files in a folder, you should see the [section below](#Applying-the-Text-Cleaning-Function-to-a-Dataframe) for how to apply the `clean_text()` function to a dataframe. 
...
# It can be useful to check that `data_clean` didn't miss any entries from `data`. You can do this by running a few `print()` statements to compare `data_clean` and `data`
...
# **A note on file size:** If you have a small number of very long input files, you may want to break those up to improve your training results. See the [section below](#Splitting-large-input-files) on why this might be necessary and how the code would work; if you choose to use this code, you should do so at this point in the text preparation process.
...
# 
# 
# - **Min_count** (minimum count) The `min_count` parameter sets how many times a word has to appear in the dictionary in order for it to 'count' as a word in the model. The default value for min_count is 5. You will likely want to change this value depending on the size of your corpus.
# 
# 
# - **Window** The `window` parameter lets you set the size of the "window" that is sliding along the text, or the number of words before and after the target word that are treated as context for training the model. The larger the window, the more words you are including in that calculation of context. Essentially, the window size impacts how far apart words are allowed to be and still be treated as relevant context.
# 
# 
...
# 
# The first step you should always take when working with models is to make sure that your model is loaded and stored within a variable for later use. We have already loaded our model above, but if you haven't already loaded your model, you should do so before proceeding.
...
# 
# One important thing to remember is that the results you get from each of these function calls do not reflect words that are _definitionally_ similar, but rather words that are used in the same **contexts**. This is an important distinction to keep in mind because while some of the words you'll get in your results are likely to be synonyms, you may have a few words in there that seem confusing if you are only expecting words with the same meanings. And, in fact, antonyms are often used in context with each other! Word embeddings guess the context of a word based on the words that often appear around it. Having an unexpected word appear in your results does not necessarily indicate that something is wrong with your model or corpus but rather may reflect that those words are used in the same way in your corpus. It always helps to go back to your corpus and get a better sense of how the language is actually used in your texts. 
# 
# ### The Code ###
# 
...
# Now that we have confirmed the term of interest is in our vocabulary, we can investigate it with a number of functions. 
# 
# **Most_similar**―this function allows you to retrieve words that are similar to chosen word. In this case, we are asking for the top ten words in our corpus that are contextually similar to the word "milk" (remembering, as discussed above, that contextually similar does not mean synonymous)!. If you want a longer list, change the number assigned to `topn` to the number of items you want in your list. You can replace "milk" with any other term you want to investigate. 
...
# 
# If you get an error message, go up and use the code above to make sure that the words you are searching are in your corpus.
# 
...
# ### Cosine Similarity ###
# 
# The way that word embedding models understand words is through their numerical representation. A word **vector** is a numerical value that represents the positioning of a word in some multi-dimensional space. 
# 
...
# 
# The larger this number is, the closer those two vectors are in vector space and thus, the more similar they are. Generally, a cosine similarity score above 0.5 tends to indicate a degree of similarity that can be considered significant. 
# 
...
#     king - man = ?
# 
# As humans, we can predict that the top word which would result from this equation would be "queen" or "princess" or even "dowager." However, because computers don't understand natural language, the computer will perform the equation by subtracting the vector for "man" from the vector for "king." 
# 
...
#  
# The equation that you could use to ask that question might be:
# 
#     bread - food = ?
# 
...
# ## Next steps ##
# 
# After you have a working model and have explored some of its functionality, it is important to evaluate the model. The question of model evaluation is a complicated one, and is discussed in detail in the [model evaluation guide](https://www.wwp.northeastern.edu/outreach/seminars/_current/handouts/word_vectors/evaluation.html) and [notebook](https://github.com/NEU-DSG/wwp-public-code-share/blob/WordVectors/WordVectors/python/wem-evaluation.ipynb). 
# 
# Since Word2Vec is an **unsupervised** algorithm—meaning the model draws its own conclusions about the data you provide—evaluation is an important step in testing the validity of the model. However, there is no single best approach to evaluating a model. While the method described in the notebook linked above offers one possible method, there are other options for conducting a model evaluation. For example, a popular method for evaluating a Word2Vec model is using the built in `evaluate_word_analogies()` function to evaluate syntactic analogies. You can also evaluate word pairs using the built in function `evaluate_word_pairs()` which comes with a default dataset of word pairs. You can read more about evaluating a model in [Gensim's documentation](https://radimrehurek.com/gensim/auto_examples/tutorials/run_word2vec.html#evaluating). 
# 

# If you would like to see other things you can do with your trained models, check out the next notebook on [visualizing word embedding data](word2vec-visualization.ipynb).

# ## Optional modifications 
# The sections below provide more information on ways that you could modify the processes for reading in and preparing texts for model training as described above. You can read more about these options here and, if you choose to use this code, you should supplement or replace the process above as indicated.

# ### Loading Data from a Spreadsheet
# 
# This step is optional for those who would prefer to work with text data in a spreadsheet rather than a folder of text files. If you want to use this code, you should run it instead of the first codeblock in the [section on loading in texts from a folder](#Code-for-Loading-Texts-from-a-Folder).
# 
...
# ### Applying the Text Cleaning Function to a Dataframe
# 
# In order to apply `clean_text` to a dataframe, you should run the code below after defining the `clean_text()` function. If you want to use this code, you should run it instead of the second codeblock in the [section on running `clean_text()`](#The-Code-for-Cleaning-Textual-Data).
# 
...
# Gensim's Word2Vec expects data to be formatted in a very specific way: a list of lists where every item in that list is a set of tokenized words (which we have created using the `clean_text()` function). It is not ideal to use a single, very large text file when using Word2Vec because of the expectation that Gensim has that data will be fed to it in this format. In short, Gensim anticipates data will be formatted in a list which holds a series of sub-lists. Those sub-lists are limited in the number of tokens Gensim will actually process. All of this happens behind the scenes and the model will seemingly run like normal when run on a single large text file, leading to some strange and deceiving results if you are unaware of these limitations. Let's explore exactly how to avoid this problem.
# 
# Gensim limits the number of tokens (remember, that a token is a single word) that will be considered in each sub-list. This means that after the first 10,000 tokens, the rest of the tokens in the sub-list will simply be ignored. So, if your data is a list which contains only a single list of more than 10,000 tokens, the other tokens will not be included in the model's training data. This doesn't mean that those words aren't included in the vocabulary, but rather that they don't contribute to the training at all and thus Word2Vec is unlikely to produce meaningful cosine similarities. 
# 
# As an example, say you have a set of novels and each novel has roughly 19,000 words each. Rather than taking all 19,000 words, the model will only be trained on the first 10,000. That is, only the first 10,000 words in the list will be used to train the model (and thus calculate semantic relationships). This would mean that almost the entire second half of the novel would not be included in this calculation. If you only have a couple of really long novels in your corpus, rather than having a very large corpus of many mid-sized files, the functional corpus of data used to train the model would actually be rather small, due to these limitations. For context, our recipe corpus is much larger than 10,000 words and even the recipe corpus is significantly smaller than what is typically used for word embeddings. 
# 
# If your corpus contains several hundred text files (and thus several hundred lists of tokens), this 10,000 word limitation doesn't matter very much as Word2Vec will ultimately receive enough tokens to build an understanding of word relationships. However, if you are only giving Word2Vec a single list of tokens, or only a handful of very long text files, Gensim's token limitations means that you are ultimately building a Word2Vec model with only 10,000 words or only a couple sets of 10,000 words, which is much too small for a proper vector space representation to function. 
# 
# Additionally, our training code is configured to ignore any word that doesn't appear at least three times in the corpus `(min_count = 3)`. This means, that if your corpus is only 19,000 words and you're only considering words that appear at least three times, then your corpus size gets even smaller. You can change `min_count` to a different number, but we advise against this as words that appear rarely in a corpus don't actually tell us very much about the corpus itself and those results may make certain words seem more significant than they actually are. 
# 
# Rather than changing `min_count`, a better solution is to split your single text file into chunks of 10,000 words so that your corpus will be formatted as a list of lists where each sub-list contains 10,000 tokens. The code which you can use to do so is provided in the cell below. 
# 
# If you don't have very large text files (or if you have a very large corpus and these limitations won't impact your model much) you shouldn't need to run this code. If you do, however, have a smaller corpus of very large text files, this code can help balance your model.
# 
# Run this code after you have read in your data and cleaned it (whether you're working with text files or a dataframe), and before you train your model. The code cell below will work if you have a single large text file. 
# 
# **Note**: If you use this code, you will also need to update the variable name in the model training code to use your new chunked text data. 

# In[ ]:


# Run this code to break a single long text into chunks
def chunks(lst, n):  # accepts a list and the number of tokens per list
   for i in range(0, len(lst), n):
       yield lst[i:i + n]

#formats the chunks of 10,000 tokens into a list to produce a list of lists
data_clean_chunked = list(chunks(data_clean[0], 10000))


# The above code will work if you have a single, large text file. If you have more than one text file, you can wrap the final line of code in a `for` loop such that the `chunks()` function is called on every item in data_clean

# In[ ]:


# Run this code to break a set of very long texts into chunks
data_clean_chunked = []
...
for i in data_clean:
    #formats the chunks of 10,000 tokens into a list to produce a list of lists
    data2 = list(chunks(i, 10000))
    data_clean_chunked.append(data2)
    data_clean_chunked = flatten(data_clean_chunked) #flatten the list of lists into a single list
