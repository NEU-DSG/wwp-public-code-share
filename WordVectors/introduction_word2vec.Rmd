...
Below is a very simple test that will calculate the cosine similarities for a small and generic set of word pairs that are likely to be related in many models. You can customize this list for your own corpus by editing the pairs below, or even adding new ones (add as many as you like, but just make sure to follow the same format as in the examples below). This code will produce a "results.csv" file with cosine similarity scores on these word pairs for every model in your folder (so, if your folder is very crowded, this will take a bit of time to run). This is meant to be an example of the kinds of testing that are used in model evaluation, and is not a substitute for more rigorous testing processes. 
