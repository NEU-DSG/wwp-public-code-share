...
# $Header:$
#
# Copyright 2018 Syd Bauman and the Northeastern University Digital
...
# a W3C-flavored regular expression that should match most any CSS3
# selector, and reject most any string that is not a CSS3 selector;
...
# check rendition/@selector; it can be used to validate itself for
# debugging.
# Alternate to STDOUT: by changing the final "print STDOUT" statement,
# you could isntead get a small XSLT program which uses said regexp to
# check rendition/@selector and the content of //*:selector which can
# be used to transform itself for debugging.
...
# to test, or leave it as $selectors for the entire thing, and issue:
# $ ~/Documents/textbase/supraSchemas/CSS3_selector_regex_generator.perl > /tmp/tcsrg.rng && java -Xss4m -jar /opt/Oxygen_XML_Editor_21/lib/oxygen-patched-jing.jar /tmp/tcsrg.rng /tmp/tcsrg.rng | egrep -v '^.?\]' | perl -pe 's,;.*$,,;' | tee /tmp/tcsrg.err | perl -pe 's,^([^:]+):([0-9]+):.*$,head -n $2 $1 | tail -n 1,;' > /tmp/tcsrg.run && source /tmp/tcsrg.run
# This gives you:
# 1) The regexp to STDERR
# 2) /tmp/tcsrg.rng: the generated RELAX NG schema
# 3) /tmp/tcsrg.err: an extract of output of running `jing` on that
#    schema against itself
# 4) /tmp/tcsrg.run: that output re-arranged into bash commands to
...
## see http://www.w3.org/TR/2011/REC-css3-selectors-20110929/ ##

# general purpose snippets
my $xmlname = "\\i\\c*";	# an XML name
my $nonascii="\&#x00A0;-\&#x10FFFF;";
my $lettersPlus="a-zA-Z_";
# define escape chars as identifier chars - (hex digit or newline)
my $neitherNewlineNorHexDigit="[$lettersPlus$nonascii-[a-fA-F0-9\&#x0A;\&#x0C;\&#x0D;]]";
# OR define as ALL chars - (hex digit or newline) ?
my $neitherNewlineNorHexDigit="[&#x21;-&#x10FFFF;-[a-fA-F0-9\\n\\r\\t]]";
my $escape="\\\\($neitherNewlineNorHexDigit|[0-9a-fA-F]{1,6}\\s?)";

# CSS 3 uses CSS 2.1 identifiers, which are very permissive
my $identinit="([$lettersPlus$nonascii]|$escape)";
my $identalso="([$lettersPlus$nonascii" . "0-9\\-]|$escape)";
my $ident="-?$identinit$identalso" . "*";

# universal and type selectors, e.g. *, *|*, duck, tei|titleStmt, *|span, svg|*
my $ns="(($ident|\\*)?\\|)?";	# namespace prefix for univ or type
my $univ="\\*";			# universal selector sans NS
my $type=$ident;		# type selector sans NS
my $univtype = "$ns($univ|$type)"; # either universal or type selector w/ NS

# attribute selectors, e.g. [type='chap'], [type~=rhyme], [xml:lang|='en']
my $attname="$ns$ident";		# attribute name
my $attrop="[\$~\*|^]?=";		# operator
...
my $sqav="\&apos;([^\&apos;]|\\\\&apos;)*\&apos;"; # single quoted attr value
my $dqav="\&quot;([^\&quot;]|\\\\&quot;)*\&quot;"; # double quoted attr value
my $attval="($sqav|$dqav|$ident)";        # attr value - an ID or a string 
my $attribute="\\[\\s*$attname\\s*($attrop\\s*$attval\\s*)?\\]"; # attribute selector

# class selectors, e.g. ".3d" -- although 10.1 of spec says it must be an identifier
my $CSS1class = "\\.\\c+";	# CSS1 allowed a class to start w/ a digit (unless followed by unit)
my $CSS2class = "\\.(\\\\[0-9]\\c*|\\c+)";	# CSS2 says an initial digit must be backslash-escaped
my $CSS21class = "\\.$ident";			# CSS2.1 says an ID
my $class=$CSS21class;

# ID selectors, e.g. "#threeDim"
my $ID = "#\\i\\c*";		# ID selector, reasonable
my $ID = "#$ident";		# ID selector in CSS 3 uses CSS 2.1 identifier
...
my $regular = '(art-lojban|cel-gaulish|no-bok|no-nyn|zh-guoyu|zh-hakka|zh-min|zh-min-nan|zh-xiang)';
my $irregular = '(en-GB-oed|i-ami|i-bnn|i-default|i-enochian|i-hak|i-klingon|i-lux|i-mingo|i-navajo|i-pwn|i-tao|i-tay|i-tsu|sgn-BE-FR|sgn-BE-NL|sgn-CH-DE)';
my $grandfathered = '(' . $irregular . '|' . $regular . ')';
my $privateUse =  '(x(-[A-Za-z0-9]{1,8}).)';
my $singleton = '[0-9A-WY-Za-wy-z]';
my $extension = '(' . $singleton . '(-[A-Za-z0-9]{2,8}).)';
my $variant = '([A-Za-z0-9]{5,8}|[0-9][A-Za-z0-9]{3})';
my $region = '([A-Za-z]{2}|[0-9]{3})';
my $script = '([A-Za-z]{4})';
my $extlang = '([A-Za-z]{3}(-[A-Za-z]{3}){0,2})';
my $language = '(([A-Za-z]{2,3}(-' . $extlang . ')?)|[A-Za-z]{4}|[A-Za-z]{5,8})';
my $langtag = '(' . $language . '(-' . $script . ')?' . '(-' . $region . ')?' . '(-' . $variant . ')*' . '(-' . $extension . ')*' . '(-' . $privateUse . ')?' . ')';
my $languageTag = '(' . $grandfathered . '|' . $langtag . '|' . $privateUse . ')';
...
my $N="\\s*([+\\-]?\\s*[0-9]*n(\\s*[+\\-]?\\s*[0-9]+)?|[+\\-]?\\s*[0-9]+|odd|even)\\s*";

my $pseudo_class_sans_not = ":(link|visited|hover|active|focus|target|lang\\($languageTag\\)|enabled|disabled|checked|root|nth(-last)?-child\\($N\\)|nth(-last)?-of-type\\($N\\)|(first|last|only)-(child|of-type)|empty)";
my $simple_selector_sans_not = "($univtype|$attribute|$class|$ID|$pseudo_class_sans_not)";
my $not = ":not\\($simple_selector_sans_not\\)";
my $pseudo_class = "$pseudo_class_sans_not|$not";
my $pseudo_element = "::?(first-(line|letter)|before|after)"; # CSS3 requires the second colon, but CSS1 and CSS2 did not

my $combinator = "[>+~&#x20;&#x09;&#x0A;&#x0D;]"; # U+0C is allowed by CSS3, but not by XML
my $simple_selector = "($univtype|$attribute|$class|$ID|$pseudo_class)"; # at the moment, unused
my $simple_sans = "($attribute|$class|$ID|$pseudo_class)"; # simple selector w/o Type and Universal selectors 
my $sequence_of_simple_selectors = "($univtype)?($simple_sans)*"; # See [1].
my $selector = "$sequence_of_simple_selectors(\\s*$combinator\\s*$sequence_of_simple_selectors)*($pseudo_element)?";
my $selectors = "$selector(,\\s*$selector)*";
# DEBUGGing can be performed by changing the value of $regexp from
# $selectors to some component thereof, e.g. $pseudo_class
my $regexp = "\\s\*$selectors\\s\*";	# we add the anchors (if needed) later
...
## main output, XSLT  (currently unused)
my $xslt = <<EXSLT;
...
$tests
    ...
    <!-- OLD DEBUGGING:
    <wc:rendition selector="*[class~='Qboth']::before, *[class~='Qbefore']::before, *[class~='Qafter']::before"/>
    <wc:rendition selector="*[class~='Qboth']::before, *[class~='Qbefore']::before"/>
    <wc:rendition selector="*[class~='Qboth']::before"/>
    <wc:rendition selector="*[class~='Qboth']:before, *[class~='Qbefore']:before, *[class~='Qafter']:before"/>
    <wc:rendition selector="*[class~='Qboth']:before, *[class~='Qbefore']:before"/>
    <wc:rendition selector="*[class~='Qboth']:before"/>
    <wc:rendition selector="*[class~='Qboth']"/>
    <wc:rendition selector=""/>
    end OLD debugging -->
  ...
  <xsl:template match="*">
    <xsl:value-of select="'selector “'
                          ||\@selector
                          ||'” is\&#x09;\&#x09;'"/>
    <xsl:if test="not( matches( \@selector, \$anchored_selector_regex,'i') )">NOT </xsl:if>
...
# Notes
# -----
# [1] Neither the grammar in 10.1 nor the prose in the 2nd para of
#     section 4 leads me to believe the (type|univ) clause is
#     optional, but it clearly is. E.g., ".class", "#id",
#     "[attr='val']", ":link", and "::after" are all valid in both
#     oXygen and Firefox.
#
# [2] To get this test suite, try something like:
#     $ wget -r -l 1 -L -np -nd https://www.w3.org/Style/CSS/Test/CSS3/Selectors/current/xml/full/flat/index.html
#     $ for f in * ; do xmlwf $f ; done
#     * Fix wellformedness errors (there are currently 4)
#     $ xsel -t -m "//css" -v "." -n -n *.xml > /tmp/duck.txt
#     Then, edit duck.txt:
#     * remove comments (there are about a dozen)
#     * remove @-lines (which are all @namespace)
#     * s/&gt;/>/g;
#     * (Do NOT s/&amp;/&/g;, though)
#     * test for unbalanced braces due to quoted '{' or '}':
#       search for "{[^{}]*{" and "}[^{}]*}", hope to find none
#     * s/{[^}]*}//g;
#     * Escape backslashes with a backslash for use here in Perl
#     * test for any apostrophes, hope for none
#     * change each remaining line to a <w3c:rendition> element, using
#       LITA around @selector value, as they use LIT a lot
# -----------------------------------------------------
...
#
# -----------------------------------------------------
# Copyright 2018 Syd Bauman and Northeastern University Digital
